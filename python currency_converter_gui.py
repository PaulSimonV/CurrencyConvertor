...
# Fetch available currency codes dynamically
...
def show_exchange_graph():
    base_currency = base_currency_var.get()
    _, rates = get_exchange_rate(base_currency, target_currency_var.get())
...
target_currency_dropdown = ttk.Combobox(root, textvariable=target_currency_var, values=CURRENCIES, state="readonly")
